...
# A fake tmux that reports FM_FAKE_PANE_PATH as the post-acquisition pane cwd
...
#   - the worktree cd send-keys and the worktree wait loop target that stable
...
test_spawn_tmux_window_construction() {
  ...
  assert_grep "send-keys -t @spawnwid cd '$wt' Enter" "$rec" \
    "worktree cd must be sent to the stable window id"
