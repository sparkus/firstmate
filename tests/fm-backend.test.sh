...
OLD_BIN_OPTIONAL_SIBLINGS="fm-pending-reply-lib.sh fm-refill-lib.sh"
...
run_spawn_case() {  # <bin-root> <fakebin> <log> <state> <data> <config> <proj> -- <spawn args...>
  # FM_FAKE_PANE_PATH (set by the caller) is both the pane cwd the fake tmux
  # reports and the path the shared lease-aware treehouse stub returns for
  # `get --lease`.
...
# old discovery poll could mistake an UNMOVED pane for one that had already
# left the project, handing validate_spawn_worktree the project's own directory
# as "the worktree" and tripping its false isolation refusal.
...
run_spawn_symlink_case() {  # <label> <physical|logical>
  ...
  out=$(FM_FAKE_PANE_PATH="$wt" run_spawn_case "$ROOT" "$fb" "$log" "$state" "$data" "$config" "$proj" -- "$id" "$proj" claude 2>&1)
  rc=$?
