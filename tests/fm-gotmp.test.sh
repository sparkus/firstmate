...
make_fake_root() {
  ...
  # Completion refill is another real teardown sibling; fm-refill-lib sources
  # fm-wake-lib, so the isolated script root must provide both files.
  ln -s "$ROOT/bin/fm-refill-lib.sh" "$fake/bin/fm-refill-lib.sh"
