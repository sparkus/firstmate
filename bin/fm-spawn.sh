...
#   terminal, so ship/scout Orca spawns do not use treehouse; cmux is a
#   session provider only, exactly like herdr/zellij, so it does. Ship and
#   scout treehouse worktrees are acquired with a durable lease held under
...
ORCA_WORKTREE_ID=
ORCA_TERMINAL=
# Set when this spawn freshly leased a treehouse ship/scout worktree; cleared
...
spawn_abort_cleanup() {
  ...
  # Release only a treehouse lease this spawn just acquired. A recovery relaunch
...
if [ "$KIND" != secondmate ] && [ "$BACKEND" != orca ]; then
  # Ship/scout worktree acquisition: durable lease under the task id, matching
  # secondmate homes (bin/fm-home-seed.sh). A plain interactive `treehouse get`
  # leaves the pool slot available, so a later get can hand the same slot to a
  ...
  if [ -n "$existing_wt" ] && [ -d "$existing_wt" ]; then
    ...
    WT=$(cd "$PROJ_ABS" && treehouse get --lease --lease-holder "$ID") || {
      echo "error: treehouse get --lease failed to lease a worktree for $ID under $PROJ_ABS" >&2
      exit 1
    }
    [ -n "$WT" ] || {
  ...
  # Enter the leased (or recovered) path in the pane. A top-level cd is visible
  # to every backend's current-path probe (unlike the old interactive get
  # subshell, which some backends cannot observe).
  spawn_send_text_line "$WT_TARGET" "cd $(shell_quote "$WT")"

  # Wait until the pane cwd matches the acquired path. Target the stable window
  # id, not the name: if the name is ever lost (e.g. an automatic-rename slips
  # through), display-message -t <bad-name> falls back to the active client's
  # window, which would misread firstmate's OWN pane path as the worktree and
  # tangle a hook into the primary checkout. The window id never lies.
  # Compare against the physical form of the leased path: a symlinked project
  # prefix would otherwise make the pane's OS-level cwd read differ from the
  # logical WT on the very first poll.
  #
  # A single matching read is not proof the pane settled: on some tmux/WSL
  # setups a brand-new window's pane_current_path transiently reports an
  # unrelated path before the shell catches up with cd. Require two consecutive
  # reads to agree on the expected leased path before accepting it.
  expected_wt_real=$(real_path_or_raw "$WT")
  candidate=""
  settled=0
  ...
  if [ "$settled" -ne 1 ]; then
    echo "error: pane did not enter leased worktree $WT within 60s; inspect window $T" >&2
    exit 1
  fi

  # Re-check isolation after the pane settled: a leased-but-wrong worktree must
  # still fail loudly rather than launch in the primary checkout.
  validate_spawn_worktree "treehouse worktree enter" "$T"
fi
