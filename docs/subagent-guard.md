...
`bin/fm-supervision-lib.sh` also recognizes X-mode relay polling and refill work as supervision needs, but unaccounted primary work still contributes nothing to that predicate.
Without an independent X-mode or refill need, unaccounted primary work therefore reads as idle rather than suspicious.
