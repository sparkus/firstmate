...
A pane can still disappear between verification and the operation; downstream submit, worktree-landing confirmation, and stale detection report that narrow race rather than treating exit 0 as success.
...
`pane_cwd` follows the top-level shell `cd` used to enter a leased worktree, while the active path reader sends begin and end markers around `pwd`, captures the marked block, and joins wrapped path lines.
Spawn uses that reader only to confirm that the pane landed on the known leased path; it is not advertised as a general live-cwd API.
